...
const Carrito = () => {
  ...
  // Leer la mesa guardada (desde Menu.jsx)
  ...
  // Asegurar que el cliente existe (por auth_user_id)
  ...
  const procederPago = async () => {
    ...
    try {
      ...
      // Leer nuevamente la mesa (por si cambió durante el proceso)
      const mesaActual = localStorage.getItem("mesa_actual")
        ? parseInt(localStorage.getItem("mesa_actual"), 10)
        : null;

      // ✅ Determinar el tipo de pedido según si hay mesa
      ...
      // Marcar mesa como ocupada si es local
      if (mesaActual) {
